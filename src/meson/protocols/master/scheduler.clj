...
(defprotocol IScheduler
  "Scheduler API Calls accepted by the master."
  (accept [this payload stream-id framework-id]
    ...
    the offers.")
  (acknowledge [this payload stream-id framework-id]
    [this payload stream-id framework-id content-type]
    "Sent by the scheduler to acknowledge a status update.")
  (acknowledge-operation-status [this payload stream-id framework-id]
    [this payload stream-id framework-id content-type]
    "Sent by the scheduler to acknowledge an operation status update.")
  (decline [this payload stream-id framework-id]
    [this payload stream-id framework-id content-type]
  ...
  (kill-task [this payload stream-id framework-id]
    [this payload stream-id framework-id content-type]
    ...
    update.")
  (message [this payload stream-id framework-id]
    [this payload stream-id framework-id content-type]
    "Sent by the scheduler to send arbitrary binary data to the executor.")
  (reconcile [this payload stream-id framework-id]
    [this payload stream-id framework-id content-type]
    "Sent by the scheduler to query the status of non-terminal tasks.")
  (reconcile-operations [this payload stream-id framework-id]
    [this payload stream-id framework-id content-type]
    "Sent by the scheduler to query the status of non-terminal and
     terminal-but-unacknowledged operations.")
  (request [this payload stream-id framework-id]
    [this payload stream-id framework-id content-type]
    "Sent by the scheduler to request resources from the master/allocator.")
  (revive [this payload stream-id framework-id]
    [this payload stream-id framework-id content-type]
    "Sent by the scheduler to remove any/all filters that it has previously
    set via `ACCEPT` or `DECLINE` calls.")
  (shutdown-executor [this payload stream-id framework-id]
    [this payload stream-id framework-id content-type]
  ...
  (suppress [this payload stream-id framework-id]
    [this payload stream-id framework-id content-type]
    "Sent by the scheduler when it doesn’t need offers for a given set of its roles.")
  (teardown [this stream-id framework-id]
    [this stream-id framework-id content-type]
    "Sent by the scheduler when it wants to tear itself down.")
  (update-framework [this payload]
    [this payload content-type]
